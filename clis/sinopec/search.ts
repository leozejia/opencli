...
const SITE = 'sinopec';
const DOMAIN = 'bidding.epec.com';
const SEARCH_ENTRIES = [
  'https://bidding.epec.com/tenderInfoOne?key=1',
  'https://bidding.epec.com/',
];
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    const rows = await searchRowsFromEntries(page, {
      ...
      allowedHostFragments: ['bidding.epec.com', 'epec.com'],
      limit,
    });
    ...
    if (rows.length === 0 && await detectAuthPrompt(page)) {
      throw new AuthRequiredError(
        DOMAIN,
        '[taxonomy=selector_drift] site=sinopec command=search login required or human verification',
      );
    }
