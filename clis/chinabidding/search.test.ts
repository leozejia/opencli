...
describe('chinabidding search helpers', () => {
  it('builds candidate URLs with keyword params', () => {
    ...
    expect(candidates.some((item) => item.includes('chinabidding.com.cn'))).toBe(true);
  });

  it('normalizes date text', () => {
    expect(__test__.normalizeDate('2026-4-8')).toBe('2026-04-08');
    expect(__test__.normalizeDate('发布时间：2026年4月8日')).toBe('2026-04-08');
  });
