...
describe('ceec search helpers', () => {
  it('builds candidate URLs with keyword params', () => {
    ...
    expect(candidates.some((item) => item.includes('ec.ceec.net.cn'))).toBe(true);
  });

  it('normalizes date text', () => {
    expect(__test__.normalizeDate('2026-4-8')).toBe('2026-04-08');
    expect(__test__.normalizeDate('公告时间：2026年04月08日')).toBe('2026-04-08');
  });
